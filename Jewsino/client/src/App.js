...
function App() {
  ...
  const [balances, setBalances] = useState({ SOL: 0, FREE: 0 });
  ...
  const handleSwitchCurrency = async (cur) => {
    await axios.post(`${API_URL}/switch-currency`, { currency: cur });
    fetchBalance();
    setResult(null);
    setPosition(null);
  };
  ...
  const handleBuy = async () => {
    ...
    try {
      ...
      if (balances[currency] < Number(amount)) throw new Error('Insufficient balance');
      // Use tradeDirection for bet direction
      const direction = tradeDirection === 'long' ? 'up' : 'down';
      await axios.post(`${API_URL}/bet`, { amount: Number(amount), direction });
      fetchBalance();
      setPosition({
  ...
  const handleSell = async () => {
    ...
    try {
      ...
      // If liquidated, lose full entry (amount)
      if ((position.direction === 'long' && currentPrice <= liq) || (position.direction === 'short' && currentPrice >= liq)) {
        pnl = -position.amount;
      } else {
      ...
      await axios.post(`${API_URL}/bet`, { amount: Math.abs(pnl), direction: pnl >= 0 ? 'up' : 'down' });
      fetchBalance();
      setResult({
        closed: true,
    ...
    <div style={{ width: '100vw', height: '100vh', background: '#181a22', color: '#f1f1f1', fontFamily: 'Inter, Segoe UI, Arial, sans-serif', overflow: 'hidden', display: 'flex', flexDirection: 'row' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100vh', minWidth: 0 }}>
          ...
          <div style={{ width: '100%', height: 54, background: 'rgba(30,34,44,0.98)', borderBottom: '1.5px solid #22242a', display: 'flex', alignItems: 'center', padding: '0 32px', boxSizing: 'border-box', zIndex: 30 }}>
            <span style={{ fontWeight: 700, fontSize: 20, letterSpacing: 0.5, color: '#f1f1f1', marginRight: 18 }}>Balance:</span>
            <span style={{ fontWeight: 700, fontSize: 20, color: '#1eea7c', marginRight: 10 }}>{balances[currency]?.toFixed(2)} {currency}</span>
          </div>
  ...
  return (
    <>
      <div style={{marginTop:8, color:'#1eea7c', fontWeight:700, fontSize:13, textAlign: 'center'}}>
        Open Position: {position.amount} {currency} @ ${position.entryPrice} ({position.leverage}x, {position.direction})
      </div>
        ...
        Liquidation Price: <span style={{color:'#fff', background:'#23272f', borderRadius:6, padding:'2px 7px', fontWeight:700}}>${liq.toFixed(2)}</span>
          ...
          {result && (
            <div style={{marginTop:8, color: result.error ? '#ff4e4e' : '#1eea7c', fontWeight:700, fontSize:13, textAlign: 'center'}}>
              {result.error ? result.error : `Closed: ${result.amount} ${currency}, PnL: $${result.pnl}`}
            </div>
          )}
        </div>
      </DraggablePanel>
